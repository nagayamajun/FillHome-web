...
import { FAIL_TO_GET_RENTALHOUSE, SUCCESS_TO_RENTALHOUSE } from "@/constants/messages";
...
export const useCreateMansionRoom = () => {
  ...
  // 型変換を行う層を検討する。
  const handleCreate = async({input, mansion_id}: {input: RoomSchemaType, mansion_id?: string}) => {
    ...
    const type_change_available_dates = Array.from(available_dates as DateObject[])
      .map((dateObject) => {
        const { year, month: { number }, day } = dateObject;
    ...
    try {
      const id = await roomFactory().create({
         input: {...rest, layout: type_change_layout , mansion_room_photos: urls, available_dates: type_change_available_dates},
         mansion_id
      });
      hideLoading();
      ...
      notice.error(`${FAIL_TO_GET_RENTALHOUSE}\n${
