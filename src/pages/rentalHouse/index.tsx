...
export const getServerSideProps: GetServerSideProps = async (
  ...
  let offset =
    ...
    (((Number(currentPage) - 1) *
      MAX_RENTAL_HOUSES_PER_REQUEST) as unknown as string);

  const response = await rentalHouseFactory().getSearch({
    search: search as string | undefined,
    limit: MAX_RENTAL_HOUSES_PER_REQUEST,
    offset,
  });
...
const RentalHousesPage = ({ response }: Props): ReactElement => {
  return (
    <SearchableRentalHouseList
      rentalHouses={response.rentalHouses}
